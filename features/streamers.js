...
module.exports= 
{
    ...
    {
        if (!(newMember.guild && newMember.voiceChannel && newMember.user.presence.game) && newMember.roles.some( i => {return i.name == "NOW LIVE!"})) return await newMember.removeRole(newMember.guild.roles.find("name", "NOW LIVE!"));
        if (!(newMember.guild && newMember.voiceChannel && newMember.user.presence.game)) return;
        if (newMember.roles.some( i => {return i.name == "KG Streamer"}) && newMember.user.presence.game.streaming && (newMember.voiceChannel.guild.name == "Konvict Gaming"))
        {
            if (!newMember.roles.some( i => {return i.name == "NOW LIVE!"}))
            { 
        ...
        else if (newMember.roles.some( i => {return i.name == "NOW LIVE!"}))
        {
